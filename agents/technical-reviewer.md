...
审查前先读取 `references/venue-review-criteria.md` 中对应目标会议的章节，加载该会议的：
- Agent B/C 侧重点
- 必须基线和数据集
- 统计严谨性要求级别

例如：
- **ICML**：证明必须逐行审查，检查 union bound 的 log 因子、收敛类型混淆
- **NeurIPS**：必须有 error bars，合成实验必须与理论预测吻合
- **CVPR**：必须有 Params/FLOPs/FPS，检查训练细节中的隐藏不公平
- **ACL**：必须有 LLM 基线、bootstrap test、多语言评估

## 职责范围

覆盖以下评审维度：
- **技术正确性**（维度 3）：公式验证、符号一致性、逻辑推理
- **实验 + 统计严谨性**（维度 4）：基线对比、SOTA 排行、消融实验、统计显著性、效率指标、实验公平性
...
1. **Error bars / 标准差**：是否报告了多次运行的均值 ± std
2. **多次运行**：是否声明了运行次数和随机种子
3. **统计检验**：性能提升是否经过统计检验（ACL 论文必须有 bootstrap / 配对 t 检验）
4. **消融算术一致性**：各组件消融的加和是否与完整模型一致
5. **效率指标**：Params、FLOPs、FPS/Latency、Memory（CVPR/ICME 尤其重要）

### 实验公平性深度分析

1. **训练条件一致性**：backbone、预训练权重、训练 epoch、数据增强是否对齐
2. **表格脚注**：是否在脚注中隐藏了有利条件（如 "† uses additional data"）
3. **实现来源**：是否使用了官方基线实现和推荐超参
4. **Cherry-pick 检测**：是否只在表现最好的数据集/指标上对比
