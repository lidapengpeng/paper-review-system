...
| `/review-paper` | Command | 用户入口，支持 `--venue` 会议适配 |
| `paper-analyzer` | Agent | PDF 解析和结构分析 |
| `technical-reviewer` | Agent | 公式验证 + 实验分析 + SOTA + 竞品对比 |
...
```bash
pip install -r /path/to/paper-review-system/requirements.txt
